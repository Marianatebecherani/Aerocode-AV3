...
const Projetos: React.FC = () => {
  ...
  const handleAddProject = (e: React.FormEvent) => {
    ...
    const statusTypeMap: Record<string, 'pending' | 'warning' | 'success' | 'error'> = {
      'Em Espera': 'pending',
      'Em Operação': 'warning',
      'Completo': 'success',
    ...
    addProject({
      title: name, // Usa title conforme a interface Project
      value,
      status,
      progress: Number(progress),
      statusType: statusTypeMap[status] || 'pending',
    });
    ...
    <div className="flex flex-col gap-8">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-400 mt-2">Visualize todos os projetos em andamento.</p>
        </div>
        <button
